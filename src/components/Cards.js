...
const Cards = (props) => {
    ...
    return(
        ...
        {props.cards.map((card, index) =>
            <Card card={card} onDelete={handleDelete} key={uuidv4()}/>
        )}
        </>
    )
}
...
class Card extends React.Component {
  render() {
    ...
    function showDeleteBtn(id) {
      document.querySelector("#deletebtn-" + id).style.display = "block";
    }

    function hideDeleteBtn(id) {
      document.querySelector("#deletebtn-" + id).style.display = "none";
    }
